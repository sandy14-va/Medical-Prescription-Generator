...
static FILE *text;
static FILE *dump=fopen("new-lambda/dump.txt","w");
...
static void state_seq() // problem 2 (viterbi)
{
	...
	fprintf(dump,"State sequence is\n\n");
    for (i = 1; i <= T; i++)
    {
        printf("%d ", Q_star[i]);
		fprintf(dump,"%d ", Q_star[i]);
    }
    printf("\n");
    q_star = 0;
}
...
static void create_lambda_new()
{
    ...
    for (itr = 1; itr <= 3; itr++)
    {
        ...
        for (obs = 1; obs <= 20; obs++)
        {
			...
			fprintf(dump,"=======================================================================================================================================\n");
            fprintf(dump,"FOR OBSERVATON SEQUENCE\n\n");
            for (i = 1; i <= T; i++)
            {
                printf("%d  ", O[i]);
				fprintf(dump,"%d  ", O[i]);
